#' lakeScale supporting functions
...
light_atten<-function(z,I0,kD){
  Iz=I0*exp(-kD*z)
  return(Iz)
}

light_atten_ts<-function(I0,kD,zmix){
  avgI=numeric(length(I0))
  for(i in 1:length(avgI)){
    avgI[i]=integrate(light_atten,0,zmix,I0=I0[i],kD=kD)$value/zmix
  }
  return(avgI)
}
...
light_profile_calc<-function(I0,z,kD=NULL,DOC=NULL,chloro=NULL,zmix=NULL){
  Ibar=NULL
  ...
  if(length(I0)==1){
    Iz=light_atten(z=z,I0=I0,kD=kD)
    if(!is.null(zmix)){
      Ibar=integrate(light_atten,0,zmix,I0=I0,kD=kD)$value/zmix
    }
  }else{
    Iz=matrix(NA,length(z),length(I0))
    Ibar=numeric(length(I0))
    for(i in 1:ncol(Iz)){
      Iz[,i]=light_atten(z,I0[i],kD)
      Ibar[i]=integrate(light_atten,0,zmix,I0=I0[i],kD=kD)$value/zmix
    }
  }
...
ts_light_profile_calc<-function(ts.data){
  data<-ts.data
  ...
  if(has.vars(data,'kD')){
    lightProf<-unlist(Map(f=light_profile_calc,I0=I0[,2],z=z[,2],kD=kD[,2],zmix=zmix[,2]))
    lightClimate<-as.numeric(lightProf[names(lightProf)=='Ibar'])
    light_z<-as.numeric(lightProf[names(lightProf)=='Iz'])
  }else if(has.vars(data,'DOC')){
    lightProf<-unlist(Map(f=light_profile_calc,I0=I0[,2],z=z[,2],DOC=DOC[,2],chloro=chloro[,2],zmix=zmix[,2]))
    lightClimate<-as.numeric(lightProf[names(lightProf)=='Ibar'])
    light_z<-as.numeric(lightProf[names(lightProf)=='Iz'])
  }
...
# daily_gpp based on hourly light, etc.
daily_gpp<-function(day,curForce,curForceDOY,chlCur,SRP,DOC,V,kD,zmix,snowDepth,PPmax=3.1,kSRP=0.3/1000,Ik=180){
  hourlyPAR=light_atten(snowDepth,curForce$I0[curForceDOY==day],kdSnow)	# umol m2 sec; SW to PAR based on Read...
  # adjusting for snow cover during winter 
    
  #### trying different GPP formulation
  PPmax=PPmax#2.2		# hr-1; 2.2 hr-1 x Chl comes from Vadeboncoeur et al. 2008 from Guildford et al 1994 
  kSRP=kSRP	# mol P m-3; Halmann and Stiller 1974 L&O 19(5): 774-783 (Lake Kinnerrett) use 0.3/1000
  Ik=Ik 		# light limitation benchmark for GPP, [umol cm-2 s-1]; Vadeboncoeur et al. 2008
  
  avgI=light_atten_ts(hourlyPAR,kD,zmix)		# hourly average light climate in mixed layer [umol cm-2 s-1]
...
# daily_gpp based on hourly light, etc.
point_gpp<-function(z,PAR,chlCur,SRP,V,kD,PPmax=3.1,kSRP=0.3/1000,Ik=180){
  # hourlyPAR=curForce$I0[curForceDOY==day]	# umol m2 sec; SW to PAR based on Read...
  
  #### trying different GPP formulation
  PPmax=PPmax #2.2		# hr-1; 2.2 hr-1 x Chl comes from Vadeboncoeur et al. 2008 from Guildford et al 1994 
  kSRP=kSRP	# mol P m-3; Halmann and Stiller 1974 L&O 19(5): 774-783 (Lake Kinnerrett)
  Ik=Ik	# light limitation benchmark for GPP, [umol cm-2 s-1]; Vadeboncoeur et al. 2008
  
  Iz=light_atten(z,PAR,kD) # hourly light at depth z [umol cm-2 s-1]
  GPPpoint=chlCur*PPmax*tanh(Iz/Ik)*((SRP/V)/((SRP/V)+kSRP))/(12*1000) # mol C m-3 day-1
  
  return(GPPpoint)
}

integrated_gpp<-function(day,curForce,curForceDOY,chlCur,SRP,DOC,V,kD,zmix,snowDepth){
  hourlyPAR=light_atten(snowDepth,curForce$I0[curForceDOY==day],kdSnow)	# umol m2 sec; SW to PAR based on Read...
  # adjusting for snow cover during winter 
  intGPP=numeric(length(hourlyPAR))
  for(i in 1:length(intGPP)){
    intGPP[i]=integrate(f = point_gpp,lower = 0,upper = zmix,PAR=hourlyPAR[i],chlCur=chlCur,SRP=SRP,V=V,kD=kD)$value
  }
  GPPout=sum(intGPP)
  return(GPPout)
}

# atmospheric deposition from Preston et al. 2008 
dep_with_dist<-function(distShore,precip,maxWind){
...
# daily tPOC deposition based on Preston et al. 2008
daily_tpoc_dep<-function(day,Area,curForce,directPrecip,nObs_hourly){
  maxWind=max(curForce$Wind[nObs_hourly==day])*(1/10)^0.15		# convert 10m wind to 1m for preston model tPOC deposition
  
  tPOCdepTotal=integrate(dep_with_dist,0.01,sqrt(Area/pi),precip=(directPrecip/Area/1000),maxWind=maxWind)$value/sqrt(Area/pi)*Area #tPOCdep summed across lake surface	#mol C day-1
  return(tPOCdepTotal)
}

# daily deposition of leaves in fall; default occurs during 2 week period in autumn; 
#  leafLoad is mol C m-1 shoreline yr-1; Likens 1985 and Gasith and Hasler 1976; Hanson et al 2014; France 1996; France and Peters 1996;  
daily_leaf_dep <- function(dateTime,startFall=275,endFall=290,leafLoad,perim){
  if(DOY(dateTime[i-1])%in%seq(startFall,endFall,1)){  # check if time step in fall period 
    dailyLeafLoad=leafLoad/length(seq(startFall,endFall,1))*perim # leaf input in mol C day-1 
  }else{
    dailyLeafLoad=0
  } 
  return(dailyLeafLoad)
}

r_func<-function(r20,temp){
  r<-r20*1.047^(temp-20) # scaled R based on Holtgrieve et al. 2010 
  return(r)
}

time_step<-function(t,out,params){
...
#     Q=X[9]
    ...
    DOCr_respired_epi=r_func(rSlow,epiTemp)
    DOCl_respired_epi=r_func(rFast,epiTemp)
    if((stage-zmix)==0){
      DOCr_respired_hypo=r_func(0,hypoTemp) # no hypo volume so making DOC respired = 0 
      DOCl_respired_hypo=r_func(0,hypoTemp) 
    }else{
      DOCr_respired_hypo=r_func(rSlow,hypoTemp) 
      DOCl_respired_hypo=r_func(rFast,hypoTemp)      
    }
    ...
    deposition=daily_tpoc_dep(i-1,area,curForce,directPrecip,nObs_hourly)    
    dailyLeafLoad=daily_leaf_dep(curForce$datetime,startFall,endFall,leafLoad,perim)
    ...
    GPP=integrated_gpp(day=i-1,curForce=curForce,curForceDOY = nObs_hourly,chlCur = chl,SRP = P_epi,
