...
### modifying to make more modular 
...
startFall = 275 # DOY when leaves start falling
endFall = 290 # DOY when leaves stop falling 
...
lake_sim<-function(cur_lake_id){
  
  cur_load_data <<- fetch_load_data(start_time, end_time, lake_id = cur_lake_id) # fetch load data 
  
  cur_geomorph <<- fetch_geomorph_data(lake_id = cur_lake_id)
  
  params <<- biogeochem_params() # 
  ...
  out=tryCatch(time_step(t=t,out=out,params=params))
...
mclapply(lakes,lake_sim,mc.cores=48)
